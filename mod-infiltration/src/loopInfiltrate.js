...
const argsSchema = [
  ...
  ['money', false] // if true, sell information instead of trading them for reputation
]
...
export async function main(ns) {
  ...
  const city = companies[target]
   const sellInformation = args.money;
    ...
    if (faction == 'none') {
      ...
      if (option) {
        await setText(option.nextSibling, faction)
         let btn;
         if (sellInformation) {
            btn = find("//button[contains(text(), 'Sell')]")
         } else {
            btn = find("//button[contains(text(), 'Trade')]")
         }
        if (btn) {
          await click(btn)
