import { ServerPool } from '/hacking/deploy-script';
...
function adjustSchedule(jobs = []) {
   ...
   for (const job of jobs) {
      if (job.startTime !== undefined && job.startTime < earliestStartTime) {
         ...
         job.args?.includes('--startTime') &&
         job.args[job.args.indexOf('--startTime') + 1] < earliestStartTime
      ) {
